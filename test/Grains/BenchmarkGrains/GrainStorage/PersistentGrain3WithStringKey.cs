    ...
    public class PersistentGrain3WithStringKey : Grain, IPersistentGrain3WithStringKey
    {
        ...
        public async Task<Report> TrySet(int index)
        {
            ...
            try
            {
                ...
                await this.persistentState.WriteStateAsync();
                sw.Stop();
                logger.LogInformation("Grain {GrainId} took {WriteTimeMs}ms to set state.", this.GetPrimaryKeyString(), sw.ElapsedMilliseconds);
                success = true;
            } catch(Exception ex)
            {
                sw.Stop();
                this.logger.LogError(ex, "Grain {GrainId} failed to set state in {WriteTimeMs}ms to set state.",  this.GetPrimaryKeyString(), sw.ElapsedMilliseconds );
                success = false;
            }
