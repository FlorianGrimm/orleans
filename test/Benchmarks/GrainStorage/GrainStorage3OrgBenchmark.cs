...
{
    public class GrainStorage3OrgBenchmark : IDisposable
    {
        ...
        public GrainStorage3OrgBenchmark(int concurrent, int payloadSize, TimeSpan duration)
        {
        ...
        public class SiloAdoNetStorageConfigurator : ISiloConfigurator
        {
            public void Configure(ISiloBuilder hostBuilder)
            {
                ...
                GrainStorageBenchmarkUtility.Prepare(TestDefaultConfiguration.DataConnectionString);
            }
        }

        public async Task RunAsync()
        {
            Orleans.Storage.AdoNetGrainStorage.HackFlag = false;
            Console.WriteLine($"{this.GetType().Name} - {Orleans.Storage.AdoNetGrainStorage.HackFlag}");
            ...
            var calltimes = reports
                .Select(r => r.Elapsed.TotalMilliseconds)
                .OrderBy(v => v).Skip(2).SkipLast(2);
            ...
            var (Json, Size) = GrainStorageBenchmarkUtility.ReadResults();
            Console.WriteLine($"Execution Plan Size {Size} kbyte.");
            //Console.WriteLine($"Execution Plan Infos: {Json}.");
        }

        public async Task<List<Report>> RunAsync(int instance, Func<bool> running)
        {
            var length = instance + 1;
            var sb = new StringBuilder();
            string instanceId = instance.ToString();
            sb.Append(instanceId);
            while (sb.Length < length) {
                sb.Append(instanceId);
            }
            instanceId = sb.Length > length ? sb.ToString(0, length) : sb.ToString();

            var persistentGrain = this.host.Client.GetGrain<IPersistentGrain3WithStringKey>(instanceId);
            // activate grain
            await persistentGrain.Init(payloadSize);
            var iteration = instance % payloadSize;
            var reports = new List<Report>(50000);
            while (running())
            {
